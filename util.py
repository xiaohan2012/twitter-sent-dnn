# TO-DO: it's good to calculate the percentage of kept words during
# normalization
import os
...
def process_stanford_sentiment_corpus(train_path, dev_path, test_path,
    ...
    with open(train_path, "r", "utf8") as train_f, \
        ...
        train_sents, train_labels = list(zip(*[sub_sent
                                               for l in train_f
                                               for sub_sent in flattened_subtrees(parse(l))]))
        dev_sents, dev_labels = list(zip(*[flatten_tree(parse(l))
                                           for l in dev_f]))
        test_sents, test_labels = list(zip(*[flatten_tree(parse(l))
                                             for l in test_f]))
    ...
    index2word = OrderedDict([(w, i)
                              for i, w in list(word2index.items())])
    ...
    train_x, train_y = create_dataset(
        train_sents, train_labels, train_sent_max_len)
...
def convert_nal_data(src_path, pkl_path):
    ...
    word2index = {
        str(row[0][0] if row[0].size > 0 else ''): i
    ...
    index2word = {
        i: word
        for word, i in list(word2index.items())
    }
