...
def get_possibility_n():
    ...
    for key in CONSTS:
        if not CONSTS[key]['on']:
            assert 'default' in CONSTS[
                key], "if ON is False, then a default must be provided"
            if 'default' in CONSTS[key]:
        ...
        if depends_on:
            ...
            if CONSTS[key].get('repeat'):
                if 'values_at_position' in CONSTS[key]:
                    for v in list(CONSTS[key]['values_at_position'].values()):
                        possibility_n *= len(v)
                possibility_n *= len(candidates)
                params[key] = tuple(
                    [random.choice(CONSTS[key]['values'])]) * dup_times
            else:
                if 'values_at_position' in CONSTS[key]:
                    value_at_positions = CONSTS[key]['values_at_position']
                    for v in list(value_at_positions.values()):
                        possibility_n *= len(v)
                    possibility_n *= (len(candidates) **
                                      (dup_times - len(value_at_positions)))
                else:
        ...
        for key in CONSTS:
            ...
            if depends_on:
                ...
                if CONSTS[key].get('repeat'):
                    ...
                    if candidates:
                        params[key] = tuple([random.choice(candidates)
                                             for _ in range(dup_times)])
                    else:
                        # fake values to be replaced
                        params[key] = tuple(range(dup_times))
            else:
            ...
            if 'values_at_position' in CONSTS[key]:
                values = [random.choice(candidates) for candidates in list(
                    CONSTS[key]['values_at_position'].values())]
                positions = list(CONSTS[key]['values_at_position'].keys())
                params[key] = modify_tuple(params[key], positions, values)

        for key in SEMI_RANDOM_PARAMS:
            # it's not used for sampling
            if not (CONSTS.get(key) and CONSTS[key]['on']):
                params[key] = SEMI_RANDOM_PARAMS[key][
                    params[semi_random_params_key]]
...
def format_params_to_cmd(name, params,
                         ...
                         more_arguments={}):
    arg_str = ' '.join(["--%s %r" % (k, v)
                        for k, v in list(more_arguments.items())])
    params_str = params2str(params)
    sig = params2str(params, cmd_sep=',,', key_val_sep='=',
                     tuple_sep=',', key_prefix='')
