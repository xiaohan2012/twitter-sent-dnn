    ...
    while len(trees) > 0:
        ...
        for t in trees:
            ...
            if new_tokens_with_labels:
                tokens, labels = list(zip(*new_tokens_with_labels))
            else:
...
def replace_tokens_by_condition(nodes, condition_func, to_token=UNK_TOKEN, to_label=3):
    ...
    """
    new_nodes = [(to_token, None, None, to_label)
                 ]  # to_token should be added also
    for node in nodes:
...
def build_node_id_mapping(nodes):
    ...
    """
    tokens = list(map(operator.itemgetter(0), nodes))
    mapping = OrderedDict()
...
def build_input(nodes, token2id):
    ...
    for t1, t2, t3, _ in nodes:
        if t1 in token2id:
            x_array.append(
                [token2id[t1], token2id.get(t2, -1), token2id.get(t3, -1)])
        else:  # cope with unknown words
            x_array.append(
                [token2id[UNK_TOKEN], token2id.get(t2, -1), token2id.get(t3, -1)])
...
def dump_data(train_path, dev_path, test_path, output_path="data/stanford_sentiment_treebank.pkl"):
    ...
    word_number = len([node for node in nodes if node[1] is None])
